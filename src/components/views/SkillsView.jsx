...
const SKILL_CATEGORIES = [
  {
    ...
    icon: <Cloud className="text-sky-400" size={16} />,
...
<span class="code-property">architectures</span>:
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-full min-h-0 overflow-y-auto p-4 sm:p-6 lg:p-8">
      ...
      {/* Visual Sphere & Badge Grid Column */}
      <div className="lg:col-span-7 flex flex-col gap-6">
        {/* Sphere Container */}
        <div className="glass-panel border-white/5 p-4 bg-slate-950/20 flex flex-col sm:flex-row items-center justify-between gap-6 relative">
          <div className="flex-1 text-center sm:text-left space-y-1.5 p-4">
            <span className="text-[10px] font-mono text-purple-400 uppercase tracking-widest block">
              3D Keyword Projection
            </span>
            <h3 className="text-lg font-bold text-gray-200 font-mono tracking-tight">
              Skill Vector Sphere
            </h3>
            <p className="text-xs text-gray-400 leading-relaxed font-light">
              Interact directly by hover-rotating or dragging points in the floating coordinate matrix on the right.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3 gap-4">
          {SKILL_CATEGORIES.map((category) => (
            <div
              key={category.title}
              className="glass-panel p-4 border border-white/5 bg-slate-950/30 flex flex-col h-full justify-between"
            >
              <div>
